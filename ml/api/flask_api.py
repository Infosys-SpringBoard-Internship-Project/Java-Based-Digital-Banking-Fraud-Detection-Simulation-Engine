...
encoders = _load_artifact(ENCODERS_PATH, lambda p: json.load(p.open('r', encoding='utf-8')))
...
def build_fraud_reason(flags, d):
    ...
    if flag.get('r12'):
        parts.append(f"R12:NewAccountLargeTransfer(age={to_int(d.get('account_age_days', 0))}days)")

    if flag.get('r13'):
        parts.append(f"R13:HighDailyVolume({to_int(d.get('txn_count_last_24hr', 0))}txns)")
...
def predict():
    ...
    features = [
        ...
        encode(d.get('merchant_category', 'retail'), merchant_map, merchant_map.get('retail', 0)),
